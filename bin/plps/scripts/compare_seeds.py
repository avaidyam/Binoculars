...
keywords = ['PLPS_path', 'receptor_file', 'ligand_dir', 'n_conf']
...
def read_input(input_file):
    ...
    return PLPS_dir, rec_file, lig_dir, n_conf
...
def main():
    if(len(sys.argv) == 2):
        ...
        print 'USAGE: python compare_seeds.py [input file]'
        exit(0)

    # read parameters and set variables for binary files
    PLPS_dir, prot_ssic_file, mol_list, max_conf = read_input(input_file)
    ...
    for mol_id in mol_list:
        ...
        for i in range(max_conf):
            if(i < 9):
                lig_ssic_file = '%s/%s_conf_0%i.ssic'%(mol_id, mol_id, i+1)
                log_file = '%s/%s_%s_conf_0%i.dat'%(mol_id, rec_pref, mol_id, i+1)
            else:
                lig_ssic_file = '%s/%s_conf_%i.ssic'%(mol_id, mol_id, i+1)
                log_file = '%s/%s_%s_conf_%i.dat'%(mol_id, rec_pref, mol_id, i+1)
            ...
            data.append(t_data)
        summary_file = '%s/%s_%s.dat'%(mol_id, rec_pref, mol_id)
