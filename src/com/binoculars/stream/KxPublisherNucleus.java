 ...
 */
public class KxPublisherNucleus<IN, OUT> extends Nucleus<KxPublisherNucleus<IN, OUT>> implements Processor<IN, OUT>, KxPublisher<OUT>, RemotedNucleus {
...
//        this._streams = streams; MUST be set sync !!
    ...
    public void subscribe(Subscriber<? super OUT> subscriber) {
        ...
        _subscribe((res, err) -> {
            ...
            Log.i(this.toString(), "stream subscribe acknowledged");
    ...
    public void _rq(long l, int id) {
        if (doOnSubscribe != null) {
            ...
            if (se != null) {// ignore unknown subscribers
                ...
                Log.w(this.toString(), "ignored credits " + l + " on id " + id);
            }
            emitRequestNext();
        }
    }
        ...
        if (lossy) {
            ...
            if (subscribers.size() > 0) {
                ...
                if (openRequested > 0 || pending.size() > 0) {
                    Log.i(this.toString(), "no subscribers, deleting " + pending.size() + " messages");
                    pending.clear();
                }
                openRequested = 0;
            }
...
//            System.out.println("stopping "+__mailbox.size()+" "+__cbQueue.size() );
    ...
    public void hasBeenUnpublished() {
        for (Iterator<Entry<Integer, SubscriberEntry>> iterator = subscribers.entrySet().iterator(); iterator.hasNext(); ) {
            ...
            Signal subscriber = entry.getValue().subscriber;
            if (subscriber instanceof SignalWrapper &&
                    ((SignalWrapper) subscriber).isTerminated()) {
                iterator.remove();
    ...
    public void subscriberDisconnected(int id) {
        Log.i(this.toString(), "a stream client disconnected id:" + id + " remaining:" + subscribers.size());
        emitRequestNext();
    }
    ...
    public KxReactiveStreams getKxStreamsInstance() {
        if (_streams == null) {
            KxReactiveStreams streams = getNucleus()._streams;
            if (streams == null) {
    ...
    protected static class KSubscription implements Subscription, Serializable {
        ...
        protected KxPublisherNucleus publisher; // actorref
        protected int id;

        public KSubscription(KxPublisherNucleus publisher, int id) {
    ...
    protected static class SubscriberEntry {
        ...
        protected Signal subscriber;

        public SubscriberEntry(int subsId, KSubscription subscription, Signal subscriber) {
            this.subsId = subsId;
