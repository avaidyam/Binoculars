...
public class Endpoint<T extends Nucleus> {
    ...
    private static HashMap<Class<? extends Nucleus>, Endpoint<? extends Nucleus>> _endpoints = new HashMap<>();
    ...
    public static <T extends Nucleus> Endpoint<T> of(Class<T> clazz) {
        Endpoint end = _endpoints.get(clazz);
        if(end != null)
            return end;

        try {
            end = new Endpoint<>(clazz);
    ...
    public static <T extends Nucleus> List<T> nodes(T node) {
        return (List<T>)Endpoint.of(node.getClass()).getNodes();
    }

    public static <T extends Nucleus> Stream<T> stream(T node) {
        return Endpoint.nodes(node).stream().parallel();
    }

    private Endpoint(Class<T> clazz) throws Exception {
        ...
        try {
            new TCPPublisher(nodes.get(0), getBroadcastPort()).publish((n) -> {
	            Log.i(PFPController.class.toGenericString(), "Disconnecting...");
	            //
            });
        } catch(Exception e) {
            e.printStackTrace();
            ...
            discover((h, p) -> {
                ...
                Log.i("[Endpoint]", "Node dropped at " + h + ":" + p);
                //
            });
        }
    }
    ...
    public void publish() throws IOException {
		...
		Log.i("[Endpoint]", "Publishing service " + broadcastName +
